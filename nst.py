...
from vgg import VGG
from losses import gram, gram_loss, total_variation_loss
import os
...
parser.add_argument("--img_dim_h", help="Height of the resulting image", type=int, default=512)
parser.add_argument("--img_dim_w", help="Width of the resulting image", type=int, default=512)
parser.add_argument("--num_iter", help="Number of iterations to run, default=10000", type=int, default=10000)
parser.add_argument("--style_loss_weight", help="Weight for the style loss, default=10e6", type=float, default=10e6)
#parser.add_argument("--content_loss_weight", help="Weight for the content loss, default=10e-4", type=float, default=10e-4)
parser.add_argument("--variation_loss_weight", help="Weight for the variation loss, default=10e3", type=float, default=10e3)
...
def main(style_img_path: str,
    ...
    transform = transforms.Compose([transforms.Resize((img_dim_h, img_dim_w)),
                                    transforms.ToTensor(), 
    ...
    noise = torch.randn(1, 3, img_dim_h, img_dim_w, device=device, requires_grad=True)
    ...
    for iteration in range(num_iter):
        ...
        total_loss = style_weight * style_loss + variation_weight * variation_loss

        if iteration % print_every == 0:
            print("Iteration: {}, Style Loss: {:.3f}, Var Loss: {:.3f}".format(iteration, style_weight * style_loss.item(), variation_weight * variation_loss.item()))
...
if __name__ == "__main__":
    ...
    img_dim_h = args.img_dim_h
    img_dim_w = args.img_dim_w
